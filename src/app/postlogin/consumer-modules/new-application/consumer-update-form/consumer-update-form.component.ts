...
@Component({
  ...
  styleUrls: ['./consumer-update-form.component.css']
})
export class ConsumerUpdateFormComponent implements OnInit {
  ...
  //   public dialogRef: MatDialogRef<ConsumerUpdateFormComponent>,
  // ) {
  ...
  individualOrGroupListStatic: Array<any> = [
    ...
    {
      "individualOrGroupId": 2, "name": "Group(Consumer)"
    }
  ];

  lastCheckboxBoolean: boolean = false
  samagraCheckboxBoolean: boolean = false;
  ...
  constructor(
    ...
    this.newApplicationService.getNatureOfWorkTypeList().pipe(takeUntil(this.unsubscribe$)).subscribe(data => {
      ...
      if (this.consumerApplicationDetail?.nscApplicationNo != null) {
        this.NatureOfworkTypeList = arr.filter((y: any) => (y.natureOfWorkTypeId == 2 || y.natureOfWorkTypeId == 5 || y.natureOfWorkTypeId == 7));
  ...
  ngOnInit() {
    ...
    this.newApplicationCreationFg = this.fb.group({
      ...
      loadRequested: ['', Validators.compose([Validators.required])],//load unit matlab numeric value
  ...
  loadFormToEdit() {
    ...
    console.log(this.loadRequestList, "loadRequestList.....");
    ...
    console.log(this.consumerApplicationDetail?.schemeType?.schemeTypeId, "this.consumerApplicationDetail?.schemeType?.schemeTypeId");
    console.log(this.newApplicationCreationFg, "this.newApplicationCreationFg..........");
    ...
    this.newApplicationCreationFg.controls['connectionType'].setValue(this.consumerApplicationDetail?.connectionType);
    ...
    if (this.consumerApplicationDetail?.loadRequestedId?.loadRequestedId) {
      console.log(this.consumerApplicationDetail?.loadRequestedId?.loadRequestedId);
      console.log(this.loadRequestList, "loadRequestList...........................");
  ...
  onChangeSelectedNatureOfWorkType(natureOfWorkTypeId: any) {
    ...
    if (natureOfWorkTypeId == 1) {
      this.individualOrGroupListStatic = this.individualOrGroupListStatic.filter(z => z.individualOrGroupId == 1 || z.individualOrGroupId == 2);
      this.newApplicationService.getSupplyVoltageList().pipe(takeUntil(this.unsubscribe$))
        .subscribe(data => {
          this.supplyVoltageList = data['list'];
        });
      ...
      if (this.consumerApplicationDetail?.nscApplicationNo != null && this.newApplicationCreationFg.get('colonyIllegalSelectionType').value == 2) {
        this.newApplicationCreationFg.get('individualOrGroupId').enable();
        this.newApplicationCreationFg.controls["individualOrGroupId"].reset();
        this.newApplicationCreationFg.get('individualOrGroupId').setValidators(Validators.required);
        this.newApplicationCreationFg.get('individualOrGroupId').updateValueAndValidity();
      ...
      this.individualOrGroupListStatic = this.individualOrGroupListStatic.filter(z => z.individualOrGroupId == 1);
      ...
      this.newApplicationCreationFg.get('loadRequestedId').enable();
      // this.newApplicationCreationFg.controls["loadRequestedId"].reset();
      this.newApplicationCreationFg.get("loadRequestedId").setValue(this.consumerApplicationDetail?.loadRequestedId?.loadRequestedId);
  ...
  samagracheckBoxCredit(value: any) {
    ...
    console.log("checkBoxCredit !!!!!!!!!!!", value.target.checked);
    if (value.target.checked) {
      this.samagraCheckboxBoolean = true;
      console.log(this.samagraCheckboxBoolean);
      this.newApplicationCreationFg.get('samagraId').enable();
      this.newApplicationCreationFg.controls["samagraId"].reset();

    }
    else {
      this.samagraCheckboxBoolean = false;
      this.newApplicationCreationFg.get('samagraId').disable();
      this.newApplicationCreationFg.controls["samagraId"].reset();
