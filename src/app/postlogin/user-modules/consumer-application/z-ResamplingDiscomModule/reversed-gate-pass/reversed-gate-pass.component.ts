...
@Component({
  selector: 'app-reversed-gate-pass',
  templateUrl: './reversed-gate-pass.component.html',
  styleUrls: ['./reversed-gate-pass.component.css']
})
export class ReversedGatePassComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ReversedGatePassComponent>,
  ...
  downloadPDF() {
    const options = {
      margin: 0.3,
      filename: 'Reverse-Gate-Pass.pdf',
    ...
    html2pdf()
      ...
      .then((pdfBlob: Blob) => {
        ...
        formDataNew.append('getRivicePassFilee', pdfBlob, `Reverse-Gate-Pass.pdf`); // Add a filename for clarity
        formDataNew.append(
          'consumerApplicationNo',
          this.myForm.value.applicationNumber || ''
        );
       
        this.consumerApplicationService.reviseGatePassUpload(formDataNew).subscribe((resp: any) => {
