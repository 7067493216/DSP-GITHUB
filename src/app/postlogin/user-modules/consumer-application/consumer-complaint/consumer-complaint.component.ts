...
@Component({
  selector: 'app-consumer-complaint',
  templateUrl: './consumer-complaint.component.html',
  styleUrls: ['./consumer-complaint.component.css']
})
export class ConsumerComplaintComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ConsumerComplaintComponent>,
  ...
  getAllComplainOfConsumer(){
    this.consumerApplicationService.getAllComplainByConsumerAndUser(this.data.consumerApplicationNo).subscribe((resp:any)=>{
      if(resp.code=="200"){
  ...
  onSubmit() {
    ...
    if (this.complainForm.invalid) {
      ...
      this.consumerApplicationService.ReplyOfConsumerComplaintByUserDiscom(this.complainForm.value).subscribe((data: any) => {
