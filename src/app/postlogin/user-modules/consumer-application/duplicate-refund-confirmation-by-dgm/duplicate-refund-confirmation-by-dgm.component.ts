...
import { DuplicatePaymentRefundPayload } from 'src/app/postlogin/consumer-modules/models/duplicatePaymentRefundPayload';

@Component({
  selector: 'app-duplicate-refund-confirmation-by-dgm',
  templateUrl: './duplicate-refund-confirmation-by-dgm.component.html',
  styleUrls: ['./duplicate-refund-confirmation-by-dgm.component.css']
})
export class DuplicateRefundConfirmationByDgmComponent implements OnInit {
  ...
  constructor(
     private consumerApplicationService: ConsumerApplicationService,
        ...
        public dialogRef: MatDialogRef<DuplicateRefundConfirmationByDgmComponent>,
...
this.notification.error("इस आवेदन पर कोई डुप्लिकेट प्रविष्टियाँ नहीं हैं।")
  ...
  onSubmit(){
    ...
    this.consumerApplicationService.updateDuplicateRefundPaymentByDgm(this.consumerApplicationDetail?.consumerApplicationNo, true, this.accessLeveOfUser?.userId, this.accessLeveOfUser?.userName, this.token).subscribe((resp:any)=>{
      if(resp?.code=="200"){
        this.notification.success(resp?.message);
        this.onClose();
      }else{
